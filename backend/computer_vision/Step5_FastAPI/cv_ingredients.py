...
FOOD_CLASSIF_MODEL = "C:\\Users\\ijzep\\_AI_\\CAPSTONE\\models\\model_FOOD_24-03_23-36_LR0.001.h5"

FOOD_CLASSIF_MODEL_CLASSES_FILE = "C:\\Users\\ijzep\\_AI_\\CAPSTONE\\models\\model_FOOD_24-03_23-36_LR0_dic.txt"
SAM_CHECKPOINT = "C:\\Users\\ijzep\\_AI_\\CAPSTONE\\models\\sam_vit_h_4b8939.pth"
...
class SingletonModelLoader:
    ...
    def get_sam_mask_generator(cls, detailed=False, mask_detail="low"):
        ...
        key = 'detailed' if detailed else 'simple'
...
def preprocess_image(img, debug=False):
    ...
    if (debug):
        ...
        cv2.imwrite(f'debug/debug_image-{randvar}.jpg',img)  # , cv2.COLOR_RGB2BGR
...
def _image_classifier(_image, _CONFIDENCE_THRESH=0.75,debug=False):
    # TODO: sacar estos de aqui. evitar accesar cada imagen
    # Global
    # 30, 23, 23
    # inside classifier
    # 31, 24, 24
    ...
    preprocessed_image = preprocess_image(_image, debug)  #TODO:  Move this step before each element?
...
def get_ingredients(image_path, detailed=False,_CONFIDENCE_THRESH=0.75, debug=False):
    ...
    def _process_found_bbox(_msk):
        ...
        if detailed:
            ...
            MAX_HEIGHT = y * 0.75  # 0.75 #545
            MAX_WIDTH = x * 0.75  # 0.75 #512
        else:
            MIN_HEIGHT = y * 0.1  # 0.1 #50
            MIN_WIDTH = x * 0.1  # 0.1 #50
            MAX_HEIGHT = y * 0.9  # 0.75 #545
            MAX_WIDTH = x * 0.9  # 0.75 #512
        ...
        for _m in _msk:
            ...
            if ((w >= MIN_WIDTH and h >= MIN_HEIGHT) and (w <= MAX_WIDTH and h <= MAX_HEIGHT)):
                ...
                # resize
                resized_img = cv2.resize(cropped__img, (224, 224)) #This happnens on the _image_class/preprocess_image as well //Todo: commented on 3003-1159 due to repetition
                _ingredient = _image_classifier(resized_img,_CONFIDENCE_THRESH=_CONFIDENCE_THRESH , debug=debug)
                #error, de tamano
                if _ingredient:
                    # print("====================================")
                    # print(f"\t\t{count}- bbox: {(_m['bbox'])}")
    ...
    #todo: Load the mask_generator directly instad of singleton
    # sam_model = SingletonModelLoader.get_sam_model()
    # if (detailed == False):
    #     _mask_generator = SamAutomaticMaskGenerator(sam_model)
    # else:
    ...
    #         min_mask_region_area=250,  # Requires open-cv to run post-processing
    #     )
    # get _mask_generator from singleton
    # TODO: comentado a las 1249 para checar si ese guarda info de la imagen

    sam_mask_generator = SingletonModelLoader.get_sam_mask_generator(detailed, mask_detail='med')
